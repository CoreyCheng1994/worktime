...
### 2.1.1 颜色使用铁律
- **同一组件必须使用同一色相**：一个组件内的背景、边框、文字必须来自同一色系，禁止跨色系混用。
- **边框颜色规则**：边框颜色必须为背景色同色系稍深 8-15% 的颜色，计算公式：`background色相 + 降低亮度/增加饱和度 = border色`。
- **禁止使用的颜色组合**：
  - 暖色背景 + 冷色边框（如 `#f4f1ea` 配 `#e5e7eb`）
  - 紫色系点缀与暖 neutral 主色调混用
  - Stone/Gray/Zinc 等冷灰与 Warm Neutral 混用
